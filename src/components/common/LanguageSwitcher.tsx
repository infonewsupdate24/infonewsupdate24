  ...
  return (
    <div ref={dropdownRef} className={`relative inline-block ${className}`}>
      ...
      <button
        ...
        className={`flex items-center gap-1.5 rounded-xl transition-all cursor-pointer select-none active:scale-95 ${
          variant === 'topbar'
            ? 'px-2.5 py-1 text-xs font-bold text-slate-700 bg-slate-100 hover:bg-slate-200 border border-slate-200 shadow-2xs'
            : variant === 'header'
      ...
      >
        <div className="flex items-center gap-1.5">
          <Globe className="h-3.5 w-3.5 text-red-600 animate-spin-slow" />
          <span>{activeOption.flagOrIcon}</span>
      ...
      {/* Dropdown Menu */}
      {isOpen && (
        <div className="absolute right-0 mt-2 w-56 rounded-2xl bg-white p-2 shadow-2xl border border-slate-200 z-50 animate-scaleUp text-xs text-slate-800">
